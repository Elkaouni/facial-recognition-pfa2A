...
##### Paths that will be used in the application. No need to change them

full_path = my_path + "\\Face_recognizer"
classifer_path= "classifier.xml"
faceCascade_path= "haarcascade_frontalface_default.xml"
user_pfp_path= my_path + "\\images\\user.png"
...
def save_history(id, happiness, surprise, sadness, fear, disgust, anger, neutrality):
    mydb=connect_to_database()
    mycursor=mydb.cursor()
    print("i m here ####################")
    sql = "select now();"
    mycursor.execute(sql)
    session_end = mycursor.fetchone()
    session_end = session_end[0]
    sql="update history_emotions set session_end = %s, rate_happy = %s, rate_surprise = %s, rate_sad = %s, rate_fear = %s, rate_disgust = %s, rate_anger = %s, rate_neutral = %s where id = %s"
    val=( session_end, happiness, surprise, sadness, fear, disgust, anger, neutrality, id)
    mycursor.execute(sql,val)
    mydb.commit()
    print("llivestreamdone ####################" + session_end)

##### Start Live Stream

def openLiveStream(window, username):
    ...
    sql = "select now();"
    mycursor.execute(sql)
    session_start = mycursor.fetchone()
    session_start = session_start[0]
    
    mycursor.execute("SELECT * from history_emotions")
    ...
    sql="insert into history_emotions( id, username, session_start) values(%s,%s,%s) "
    val=(id, username, session_start)
    mycursor.execute(sql,val)
    mydb.commit()
    ...
    def openMenu(username):
        ...
        #save_history(id, happiness, surprise, sadness, fear, disgust, anger, neutrality) 
        ...
        y_button = Button(frame23, text="Start now", font=("Courrier", 25), bg='#0F094A', fg='#ffffff', command = lambda : openLiveStream(window, username))
    ...
    exit_button = Button(root, text="Exit", font=("Courrier", 25),bg='#0F094A', fg='#ffffff', command= lambda : openMenu(username))
